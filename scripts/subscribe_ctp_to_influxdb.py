...
if __name__ == '__main__':
    ...
    opt.add_argument('--model', default='influx', help='可选 mongodb 或者 influx')
    ...
    opt.add_argument('--db_uri', help='like influxdb://user:password@127.0.0.1:8086/database_name')
    ...
    MarketDataFacade.to_db(user=args.user, password=args.password, broker=args.broker,
                           front=args.front,
                           instrument_ids=instrument_ids, db_uri=args.db_uri, worker=args.worker,
                           trade_front=args.trade_front, model=args.model)
